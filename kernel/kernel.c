...
static void print_splash(void) {
    ...
    vga_set_cursor(2, 2);
    vga_puts_color("  Stage 1: Multi-tasking & Preemptive Round-Robin Scheduling", VGA_LIGHT_CYAN, VGA_BLACK);

    vga_set_cursor(3, 2);
    vga_puts_color("  Faculty of Engineering - Department of Software Engineering", VGA_LIGHT_GREY, VGA_BLACK);

    vga_set_cursor(4, 2);
    vga_puts_color("  Built by students, for students.  Type 'help' to begin.", VGA_LIGHT_GREEN, VGA_BLACK);

    vga_set_cursor(5, 2);
    vga_puts_color("  CPU: i686 (32-bit Protected Mode)  |  Display: VGA 80x25", VGA_DARK_GREY, VGA_BLACK);
...
void kernel_main(void) {
    ...
    // Create background tasks (PID 1 and PID 2)
